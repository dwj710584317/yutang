...
public class JsonUtil {
    public String getPhoneJson(List<String> lal, String edtPhone, List<PhoneDto> phoneContactsList, List<SmsDto> smsList, String userMake){
        String jsonSimPhone="0";
        ...
        if (edtPhone.equals("") || edtPhone.isEmpty()){
            jsonSimPhone="0";
        }else {
            jsonSimPhone=edtPhone.trim();
        }


        javaBean.setMobile(edtPhone.trim());
        ...
        if (phoneContactsList.size()>=1&&smsList.size()>=1) {
            for (int i = 0; i < phoneContactsList.size(); i++) {   //通讯录
                ...
                strJsonDataBean.setMobile(jsonSimPhone);
                strJsonDataBean.setPedvice(phoneContactsList.get(i).getTelPhone().trim());
            ...
            for (int i = 0; i < smsList.size(); i++) {     //短信
                ...
                strJsonDataBean.setMobile(jsonSimPhone);
                strJsonDataBean.setPedvice(smsList.get(i).getAddress());
